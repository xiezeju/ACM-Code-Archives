...
public:
    void dfs(Node* node){
        ...
        Node* Next = node->next; //在找到child节点的时候会将next指向child节点，不保存的话回来就找不到原先的next节点了
        node->prev = p; //此处的p是上一次记录的节点
        if(p!=nullptr) p->next = node;
        p = node; //p指向当前节点
        if(node->child!=nullptr){
            dfs(node->child);
            node->child=nullptr;//记得将child置空
        }
        if(Next!=nullptr) dfs(Next);
    }
...
private:
    Node* p = nullptr; //p为优先深度遍历到的最后一个节点
};
// @lc code=end
