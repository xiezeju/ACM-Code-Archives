 ...
 * f1_content, f2_content ... fn_content ) 。注意：n >= 1 且 m >= 0 。如果 m = 0
 * ，则表示该目录是根目录。
 * 
 ...
 * 
 * 
 * 输入：paths = ["root/a 1.txt(abcd) 2.txt(efgh)","root/c 3.txt(abcd)","root/c/d
...
public:
    vector<vector<string>> findDuplicate(vector<string>& paths) {
        ...
        for(auto& path:paths){
            int i = 0, j = 0;
            path += " ";
            string pre = "", file = "",content = "";
            while(path[i]!=' ') pre += path[i++];
            i++;
            vector<string> files;
            while(i<path.size()){
                if(path[i] == ' '){
                    s[content].push_back(pre+'/'+file);
                    file.clear();
                    content.clear();
                    j = 0;
                }
