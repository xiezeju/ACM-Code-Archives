 ...
 * 在第 1 天，你花了 costs[0] = $2 买了一张为期 1 天的通行证，它将在第 1
 * 天生效。 在第 3 天，你花了 costs[1] = $7 买了一张为期 7 天的通行证，它将在第
 * 3, 4, ..., 9 天生效。 在第 20 天，你花了 costs[0] = $2 买了一张为期 1
            ...
            for (int i = 1; i <= days[n - 1]; i++) {
                if (dp[i] == -1) {
                    dp[i] = min(dp[i - 1] + cost[0],
                                min(dp[i - 7 >= 0 ? i - 7 : 0] + cost[1],
                                    dp[i - 30 >= 0 ? i - 30 : 0] + cost[2]));
                } else {
                    dp[i] = dp[i - 1];
                }
            }
