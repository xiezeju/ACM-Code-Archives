 ...
 *
 * 就像 之前的问题 那样，给定的树是利用 Construct(a) 例程从列表 a（root =
 * Construct(a)）递归地构建的：
 *
 *
 * 如果 a 为空，返回 null 。
...
class Solution {
    public:
        ...
        TreeNode* build(TreeNode* p, int l, int r) {
            if (l > r) {
                return nullptr;
            }
            ...
            return new TreeNode(max_num, build(p, l, index - 1),
                                build(p, index + 1, r));
        }
