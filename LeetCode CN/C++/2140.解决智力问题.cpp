 ...
 * brainpoweri] 。
 *
 * 这个数组表示一场考试里的一系列题目，你需要 按顺序 （也就是从问题
 * 0 开始依次解决），针对每个问题选择 解决 或者 跳过 操作。解决问题 i 将让你
 * 获得  pointsi 的分数，但是你将
 * 无法 解决接下来的 brainpoweri 个问题（即只能跳过接下来的 brainpoweri
...
class Solution {
    public:
        long long mostPoints(vector<vector<int>>& questions) {
            ...
            // dp[i]代表解决问题第i道题及以后的题能获得的最大分数
            vector<long long> dp(n + 1);

            for (int i = n - 1; i >= 0; i--) {
                dp[i] = max(dp[i + 1], questions[i][0] +
                                           dp[min(n, i + questions[i][1] + 1)]);
            }
