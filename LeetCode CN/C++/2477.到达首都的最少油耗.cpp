 ...
 *
 * 给你一棵
 * n 个节点的树（一个无向、连通、无环图），每个节点表示一个城市，编号从 0 到 n -
 * 1 ，且恰好有 n - 1 条路。0 是首都。给你一个二维整数数组 roads ，其中 roads[i]
 * = [ai, bi] ，表示城市 ai 和 bi 之间有一条 双向路 。
 *
...
class Solution {
    public:
        ...
        int dfs(int x, int fa, int seats) {
            for (int& y : g[x]) {
                if (y == fa)
                    continue;
                int = dfs(y, x, seats);
