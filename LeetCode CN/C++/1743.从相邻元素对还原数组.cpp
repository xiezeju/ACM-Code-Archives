 ...
 * = [ui, vi] 表示元素 ui 和 vi 在 nums 中相邻。
 *
 * 题目数据保证所有由元素 nums[i] 和 nums[i+1] 组成的相邻元素对都存在于
 * adjacentPairs 中，存在形式可能是 [nums[i], nums[i+1]] ，也可能是 [nums[i+1],
 * nums[i]] 。这些相邻元素对可以 按任意顺序 出现。
 *
...
class Solution {
    public:
        ...
        void dfs(int head, int from) {
            a.push_back(head);
            if (g[head].size() == 1 && head != from) {
                return;
            }
