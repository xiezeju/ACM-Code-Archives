 ...
 * 之间的唯一编号。给你一个二维数组 graph ，其中 graph[u] 是一个节点数组，由节点
 * u 的邻接节点组成。形式上，对于 graph[u] 中的每个 v ，都存在一条位于节点 u
...
class Solution {
    public:
        bool dfs(vector<vector<int>>& graph, vector<int>& color, int id,
            ...
            if (color[id] != -1 && color[id] != cur_color) {
                return false;
            }
            ...
            for (auto& node : graph[id]) {
                if (color[node] == (cur_color ^ 1)) {
                    continue;
                }
                if (!dfs(graph, color, node, cur_color ^ 1)) {
                    return false;
                }
            }
        ...
        bool isBipartite(vector<vector<int>>& graph) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (!dfs(graph, color, i, cur_color)) {
                    return false;
                }
            }
            return true;
