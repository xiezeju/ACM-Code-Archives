/*
 * @lc app=leetcode.cn id=236 lang=cpp
 *
 * [236] 二叉树的最近公共祖先
 *
 * https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-tree/description/
 *
 * algorithms
 * Medium (70.14%)
 * Likes:    2510
 * Dislikes: 0
 * Total Accepted:    610.4K
 * Total Submissions: 869.2K
 * Testcase Example:  '[3,5,1,6,2,0,8,null,null,7,4]\n5\n1'
 *
 * 给定一个二叉树, 找到该树中两个指定节点的最近公共祖先。
 *
 * 百度百科中最近公共祖先的定义为：“对于有根树 T 的两个节点
 * p、q，最近公共祖先表示为一个节点 x，满足 x 是 p、q 的祖先且 x
 * 的深度尽可能大（一个节点也可以是它自己的祖先）。”
 *
 *
 *
 * 示例 1：
 *
 *
 * 输入：root = [3,5,1,6,2,0,8,null,null,7,4], p = 5, q = 1
 * 输出：3
 * 解释：节点 5 和节点 1 的最近公共祖先是节点 3 。
 *
 *
 * 示例 2：
 *
 *
 * 输入：root = [3,5,1,6,2,0,8,null,null,7,4], p = 5, q = 4
 * 输出：5
 * 解释：节点 5 和节点 4 的最近公共祖先是节点 5
 * 。因为根据定义最近公共祖先节点可以为节点本身。
 *
 *
 * 示例 3：
 *
 *
 * 输入：root = [1,2], p = 1, q = 2
 * 输出：1
 *
 *
 *
 *
 * 提示：
 *
 *
 * 树中节点数目在范围 [2, 10^5] 内。
 * -10^9
 * 所有 Node.val 互不相同 。
 * p != q
 * p 和 q 均存在于给定的二叉树中。
 *
 *
 */

// @lc code=start
/**
 * Definition for a binary tree node.
 * struct TreeNode {
 *     int val;
 *     TreeNode *left;
 *     TreeNode *right;
 *     TreeNode(int x) : val(x), left(NULL), right(NULL) {}
 * };
 */
/*递归做法
class Solution {
    public:
        TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p,
                                       TreeNode* q) {
            // 找到p或者q就返回
            if (root == nullptr || root == p || root == q) {
                return root;
            }

            TreeNode* left = lowestCommonAncestor(root->left, p, q);
            TreeNode* right = lowestCommonAncestor(root->right, p, q);

            // 以当前节点为根的左子树中没有找到p和q，说明可能在右子树中
            if (left == nullptr) {
                return right;
            }

            // 和上面的情况相反，可能在左子树中
            if (right == nullptr) {
                return left;
            }

            // 如果左右子树中都找到了，说明root是q和p的最近公共祖先
            return root;
        }
};
*/

class Solution {
    public:
        typedef pair<bool, bool> P;

        // 后序遍历查找LCA
        P LCA(TreeNode* root, TreeNode* p, TreeNode* q, TreeNode*& lca) {
            P result(false, false);
            P left(false, false);
            P right(false, false);

            // 后序遍历左右子树，寻找p和q并标记
            if (root->left) {
                left = LCA(root->left, p, q, lca);
                if (lca)
                    return left;
            }
            if (root->right) {
                right = LCA(root->right, p, q, lca);
                if (lca)
                    return right;
            }

            // 标记左子树中是否找到p，右子树中是否找到q
            result.first = left.first || right.first;
            result.second = left.second || right.second;

            // 当前节点是p或者q就标记
            if (root == p) {
                result.first = true;
            }
            if (root == q) {
                result.second = true;
            }

            // 找到最近公共祖先
            if (!lca && result.first && result.second) {
                lca = root;
            }
            return result;
        }

        TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p,
                                       TreeNode* q) {
            if (root == nullptr) {
                return nullptr;
            }

            TreeNode* lca = nullptr;
            LCA(root, p, q, lca);

            return lca;
        }
};

// @lc code=end
