 ...
 * 输出：[2]
 * 解释：树中最深的节点为 2 ，有效子树为节点 2、1 和 0 的子树，但节点 2
 * 的子树最小。
 *
 *
 *
...
class Solution {
 public:
  pair<TreeNode*, int> LCA(TreeNode* p) {
    if (p == nullptr) {
      return {p, 0};
    }
    auto left = LCA(p->left);
    auto right = LCA(p->right);
  ...
  TreeNode* subtreeWithAllDeepest(TreeNode* root) { return LCA(root).first; }
};
// @lc code=end
